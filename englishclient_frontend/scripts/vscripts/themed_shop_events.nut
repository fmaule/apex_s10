...
global function ThemedShopEvent_GetWhatsNewList
#endif
...
#if SERVER || CLIENT || UI
asset function ThemedShopEvent_GetWhatsNewList( ItemFlavor event )
{
	Assert( ItemFlavor_GetType( event ) == eItemType.calevent_themedshop )
	return GetGlobalSettingsAsset( ItemFlavor_GetAsset( event ), "whatsNewItemFlavorList" )
}
#endif
