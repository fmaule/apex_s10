...
void function HeirloomShopPanel_UpdateGRXDependantElements()
{
	...
	if ( menuIsUsable )
	{
		...
		array<GRXScriptOffer> offers = GRX_GetLocationOffers( themeStruct.grxOfferLocation )
		...
		foreach ( offerIndex, offer in offers )
		{
			...
			ItemFlavor meleeSkin = ItemFlavorBag_GetMeleeSkinItem( offer.output )
			ItemFlavor character = MeleeSkin_GetCharacterFlavor( meleeSkin )
			bool isOwned = GRX_IsItemOwnedByPlayer_AllowOutOfDateData( meleeSkin )

			HudElem_SetRuiArg( offerButton, "itemName", ItemFlavor_GetLongName( character ) )
			HudElem_SetRuiArg( offerButton, "itemDesc", isOwned ? "#COLLECTED" : ItemFlavor_GetLongName( meleeSkin ) )
			HudElem_SetRuiArg( offerButton, "itemRarity", ItemFlavor_GetQuality( meleeSkin ) )
			HudElem_SetRuiArg( offerButton, "itemImg", MeleeSkin_GetStoreImage( meleeSkin ), eRuiArgType.IMAGE )
...
void function OfferButton_OnActivate( var btn )
{
	...
	ItemFlavor offerFlav = ItemFlavorBag_GetMeleeSkinItem( offer.output )
