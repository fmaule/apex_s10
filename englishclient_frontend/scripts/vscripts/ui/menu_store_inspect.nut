...
void function UpdatePrices( GRXScriptOffer storeOffer, bool isBundlesRefresh )
{
	...
	else
	{
		...
		if ( storeOffer.prices.len() == 2 )
		{
			...
			s_inspectOffers.displayedPriceStr = Localize( "#STORE_PRICE_N_N", firstPrice, secondPrice )
...
void function StoreInspectMenu_UpdatePurchaseButton( GRXScriptOffer storeOffer )
{
	...
	if ( storeOffer.prereq != null )
	{
		...
		if ( ItemFlavor_GetType( prereqFlav ) == eItemType.calevent_collection )
		{
			...
			s_inspectOffers.purchaseText = Localize( "#LOCKED" )
			#if DEV
				s_inspectOffers.purchaseDescText = Localize( "#STORE_REQUIRES_LOCKED", "PLEASE FIX ME FOR S11" )
			#else
				s_inspectOffers.purchaseDescText = Localize( "#STORE_REQUIRES_LOCKED", Localize( "#EVOLUTION_COLLECTION" ) )                                                                                 
			#endif
		}
...
void function StoreInspectMenu_UpdateButtonForEquips()
{
	...
	if ( itemType == eItemType.weapon_charm || itemType == eItemType.account_pack || itemType == eItemType.emote_icon )
	{
	...
	{
		RuiSetString( rui, "buttonDescText", Localize( "#SKYDIVE_EQUIP_INSTRUCTIONS" ) )
		s_inspectOffers.isOwnedItemEquippable = false
	}
