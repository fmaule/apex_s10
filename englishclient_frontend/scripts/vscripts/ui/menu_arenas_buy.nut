...
void function ClientToUI_Arenas_RefreshBuyMenu()
{
	...
	RuiSetString( Hud_GetRui( file.abilityLabel ), "labelText", Localize( "#ARENAS_SHOP_LEGEND_ABILITIES", Localize( ItemFlavor_GetLongName( character ) ).toupper() ) )
		...
		if ( GetCurrentPlaylistVarString( "arenas_takeover_name", "" ) == "Rampart" )
		{
			RuiSetImage( Hud_GetRui( file.leftFrame ), "basicImage", $"rui/arenas/arenas_store_purchase_frame_l_rampart" )
			RuiSetImage( Hud_GetRui( file.rightFrame ), "basicImage", $"rui/arenas/arenas_store_purchase_frame_r_rampart" )
			RuiSetBool( Hud_GetRui( Hud_GetChild( file.menu, "NarratorImage" ) ), "isVisible", true )
		}
		else
		{
