...
global function PromoDialog_NumPages
global function PromoDialog_GetPages

const PROMO_DIALOG_MAX_PAGES = 9
const string PIN_MESSAGE_TYPE_PROMO = "motd"

global struct PromoDialogPageData
{
	asset  image = $""
	string imageName = ""
	string title = ""
...
bool function OpenPromoDialogIfNew()
{
	...
	if ( GetConVarBool( "assetdownloads_enabled" ) )
	{
		...
		{
			string promoRpakName = EADP_IsUMEnabled() ? file.pages[0].imageName : GetPromoRpakName()
			RunClientScript( "RequestDownloadedImagePakLoad", promoRpakName, ePakType.DL_PROMO, Hud_GetChild( file.menu, "ActivePage" ), file.pages[0].imageName )
			RunClientScript( "RequestDownloadedImagePakLoad", promoRpakName, ePakType.DL_PROMO, Hud_GetChild( file.menu, "LastPage" ), file.pages[0].imageName )
		}
	}
...
{
	if ( GetLocalClientPlayer() != null && IsFullyConnected() && file.hijackContent == null )
	{
...
void function PromoDialog_InitPages()
{
	...
	if ( EADP_IsUMEnabled() )
	{
		array<PromoDialogPageData> umPages = PromoDialog_GetPagesFromUM()
		file.pages.extend( umPages )
		return
	}
...
array<PromoDialogPageData> function PromoDialog_GetPages()
{
	return file.pages
}
...
void function UpdatePageRui( var rui, int pageIndex )
{
	...
	if( GetConVarBool( "assetdownloads_enabled" ) && file.hijackContent == null )
	{
		string promoRpakName = EADP_IsUMEnabled() ? page.imageName : GetPromoRpakName()
		RuiSetImage( rui, "imageAsset", GetDownloadedImageAsset( promoRpakName, page.imageName, ePakType.DL_PROMO ) )
		RuiSetBool( rui, "isImageLoading", IsImagePakLoading( promoRpakName ) )
	}
	else
	{
