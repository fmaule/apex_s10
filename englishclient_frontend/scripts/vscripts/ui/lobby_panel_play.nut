...
global function ToggleRavenStatus
...
void function InitPlayPanel( var panel )
{
	...
	Hud_AddEventHandler( rankedBadge, UIE_CLICK, OpenRankedInfoPage )
		...
		file.storyEventButton0 = Hud_GetChild( panel, "StoryEventButton0" )
...
void function UpdateModeButton()
{
	...
	bool useGamemodeSelect = GamemodeSelect_IsEnabled() && !(ShouldDisplayOptInOptions() && uiGlobal.isOptInEnabled)
	...
	if ( useGamemodeSelect && playlistName != "" )
	{
		...
		if ( PlaylistHasRotationGroup( Lobby_GetSelectedPlaylist() ) )
		{
			...
			HudElem_SetRuiArg( file.gamemodeSelectButton, "modeDescText", descText )
		}
	}
...
void function UpdateLootBoxButton( var button, array<ItemFlavor> specificPackFlavs = [] )
{
	...
	if ( packFlav != null )
	{
		...
		if ( GetGlobalSettingsString( ItemFlavor_GetAsset( packFlav ), "packType" ) == "EVENT_PACK" )
		{
			buttonText = ItemFlavor_GetShortName( packFlav )
			descText = (lootBoxCount == 1 ? "#EVENT_PACK" : "#EVENT_PACKS")
		}
		else if ( GetGlobalSettingsString( ItemFlavor_GetAsset( packFlav ), "packType" ) == "THEMATIC_PACK" || GetGlobalSettingsString( ItemFlavor_GetAsset( packFlav ), "packType" ) == "EVENT_THEMATIC_PACK" )
		{
			buttonText = ItemFlavor_GetShortName( packFlav )
...
{
	file.currentMaxMatchmakingDelayEndTime = SharedRanked_GetMaxPartyMatchmakingDelay() + Time()
}
...
bool function Lobby_ShowStoryEventChallengesPopup( bool forceShow = false )
{
	...
	foreach ( event in storyChallengeEvents )
	{
		...
		thread function() : ( hasSeenPersistenceVarNames, eventTitle, eventDesc, eventType )
		{
			...
			HudElem_SetRuiArg( popup, "unlockedString", "" )
			HudElem_SetRuiArg( popup, "buttonImage", $"rui/events/s10e04/challenges_logo", eRuiArgType.IMAGE )                                                        
			HudElem_SetRuiArg( popup, "forceFullIcon", false )
...
void function ToggleRavenStatus( bool present )
{
	file.isRavenPresent = present
}

void function UpdateStoryButton()
{
	if ( file.isRavenPresent )
	{
		Hud_SetVisible( file.storyEventButton0, true )
		Hud_SetEnabled( file.storyEventButton0, true )
	}
	else
	{
		Hud_SetVisible( file.storyEventButton0, false )
		Hud_SetEnabled( file.storyEventButton0, false )
	}
}

void function StoryModeButton_OnGetFocus( var button )
{
	RunClientScript( "S10E04_ToggleHighlightOnRaven", true )
}

void function StoryModeButton_OnLoseFocus( var button )
{
	RunClientScript( "S10E04_ToggleHighlightOnRaven", false )
}
...
void function StoryModeButton_OnActivate( var button )
{
	...
	thread function() : ( button )
	{
		...
		RunClientScript( "S10E04_TriggerCrowLeave" )

		ItemFlavor ornull storyChallengeEventOrNull = GetStoryChallengeEventIfActive( GetUnixTimestamp(), "calevent_s10e04_story_challenges" )
