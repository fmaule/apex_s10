...
array<MiniPromoPageData> function InitPages()
{
	if ( EADP_IsUMEnabled() )
	{
		return MiniPromo_GetPageDataFromUM()
	}
...
array<MiniPromoPageData> function MiniPromo_GetPageDataFromUM()
{
	array<MiniPromoPageData> pages
...
void function SetPage( int pageIndex, bool instant = false )
{
	...
	string activePageRpakName = EADP_IsUMEnabled() ? page.imageName : GetMiniPromoRpakName()
	if( GetConVarBool( "assetdownloads_enabled" ) && file.activePageIndex > 0 )
		RuiSetImage( rui, "imageAsset", GetDownloadedImageAsset( activePageRpakName, page.imageName, ePakType.DL_MINI_PROMO, file.button ) )
	else
	...
	string lastPageRpakName = EADP_IsUMEnabled() ? lastPage.imageName : GetMiniPromoRpakName()
	if( GetConVarBool( "assetdownloads_enabled" ) && lastActivePage > 0 )
		RuiSetImage( rui, "lastImageAsset", GetDownloadedImageAsset( lastPageRpakName, lastPage.imageName, ePakType.DL_MINI_PROMO ) )
	else
	...
	RuiSetBool( rui, "largeImageMode", EADP_IsUMEnabled() )
	...
	if ( ownedPacks > 0 )
	{
		...
		string nextPackDescription = packIcon == "" ? "#APEX" : ItemFlavor_GetShortName( pack )
		if ( packIcon != "" && GetGlobalSettingsString( ItemFlavor_GetAsset( pack ), "packType" ) == "APEX_PACK" )                                                                                          
...
ToolTipData function GetPackInfoToolTip( int ownedPacks )
{
	...
	{
		if ( GetGlobalSettingsString( ItemFlavor_GetAsset( counterPack ), "packType" ) == "EVENT_PACK" )
		{
			evtPackCnt += GRX_GetPackCount( ItemFlavor_GetGRXIndex( counterPack ) )
		}
		else if ( GetGlobalSettingsString( ItemFlavor_GetAsset( counterPack ), "packType" ) == "THEMATIC_PACK" || GetGlobalSettingsString( ItemFlavor_GetAsset( counterPack ), "packType" ) == "EVENT_THEMATIC_PACK" )
		{
...
void function MiniPromoButton_OnActivate( var button )
{
	...
	else if ( EADP_IsUMEnabled() && page.linkType == "openmotd" )
	{
