...
bool function PrivateMatch_TeamRoster_Configure( PlacementStruct teamPlacement, string teamName, array<PrivateMatchStatsStruct> playerTeamStatsArray )
{
	int placement = -1
...
void function PrivateMatch_TeamPlacement_Update( table< int, array< PrivateMatchStatsStruct > > teamPlacementMap )
{
	...
	foreach ( teamIndex, teamPlacement in file.teamPlacement )
	{
		...
		if ( !PrivateMatch_TeamRoster_Configure( teamPlacement, "ASDF", playerTeamStatsArray ) )
		{
			continue
		}	
