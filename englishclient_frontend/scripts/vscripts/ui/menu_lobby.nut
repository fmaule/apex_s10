...
void function OnGRXStateChanged()
{
	...
	if ( ready )
	{
		...
		if ( GetPersistentVar( "s10StoryEvent.hasStep4Completed" ) && !GetPersistentVar( "s10StoryEvent.hasSeenFinaleReward" ) )
		{
			array<BattlePassReward> rewards
			BattlePassReward info
			info.level = -1
			info.flav = GetItemFlavorByHumanReadableRef("pack_thematic_bloodhound")
			info.quantity = 1
			rewards.append( info )
			rewards.append( info )
			rewards.append( info )
			...
			ShowRewardCeremonyDialog(
				"",
				"#S10E04_CHALLENGE_4_REWARD_TITLE",
				"#S10E04_CHALLENGE_4_REWARD_DESCRIPTION",
				rewards,
				false,
				true,
				true,
			...
			Remote_ServerCallFunction( "ClientCallback_S10E04_MarkStoryFinaleRewardSeen" )
		}
...
void function UpdateCornerButtons()
{
	...
	Hud_SetVisible( file.newsButton, NewsButton_IsVisible() )
	Hud_SetVisible( file.newsButtonStatusIcon, NewsButton_IsVisible() )
...

	{
		...
		if(datacenterButtonVisible)
			Hud_SetPinSibling(file.bonusXp, "DatacenterButton")
		else
...
bool function NewsButton_IsVisible()
{
