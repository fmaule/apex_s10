...
struct {
	...
	var todaysRewardButton
	BattlePassReward& todaysRewardData
...
void function InitQuestPanel( var panel )
{
	...
	SetPanelTabTitle( panel, "#TREASURE_PACK_TAB_TITLE" )
...
void function ComicPageButtonOnClick( var button )
{
	...
	int status = SeasonQuest_GetStatusForComicPageIndex( GetLocalClientPlayer(), quest, startPageIndex )
	...
	if ( status >= eQuestMissionStatus.LAUNCHABLE )
	{
		...
		int pageCount = SeasonQuest_GetComicPagesUnlockedByPlayer( GetLocalClientPlayer(), quest )
...
void function SetupButtonEvents( var panel )
{
	...
	file.todaysRewardButton =  Hud_GetChild( panel, "TodaysReward" )

	Hud_AddEventHandler( file.todaysRewardButton, UIE_CLICK, QuestPanel_TodaysRewardButton_OnActivate )
...
void function QuestPanel_TodaysRewardButton_OnActivate ( var button )
{
	if ( Hud_IsLocked( button ) )
		return

	if ( ItemFlavor_GetType( file.todaysRewardData.flav ) == eItemType.loadscreen )
	{
		LoadscreenPreviewMenu_SetLoadscreenToPreview( file.todaysRewardData.flav )
		AdvanceMenu( GetMenu( "LoadscreenPreviewMenu" ) )
		return
	}
	else if ( ItemFlavor_GetType( file.todaysRewardData.flav ) == eItemType.voucher )
	{
		return
	}
	else if ( InspectItemTypePresentationSupported( file.todaysRewardData.flav ) )
	{
		RunClientScript( "ClearBattlePassItem" )
		SetTreasurePackItemPresentationModeActive( file.todaysRewardData )
		return
	}
}
...
void function UpdateComicPageButtons( var panel, bool haveStartedQuest )
{
	...
	foreach ( int comicPageIndex, var comicPageButton in file.comicPageButtons )
	{
		...
		float unlockTime = ClientTime() + (SeasonQuest_GetUnlockTimeForComicIndex( quest, comicPageIndex ) - GetUnixTimestamp())
...
void function UpdateRewardTrackButtons( var panel, bool startedQuest, int treasureBoxesRemaining )
{
	...
	var todaysButton  		= file.todaysRewardButton

	            
	if(treasureBoxesRemaining == 0)
	{
		Hud_Hide( file.todaysRewardButton )
	}
	else
	{
		Hud_Show( file.todaysRewardButton )
		ItemFlavorBag todaysRewardBag = SeasonQuest_GetTreasurePackRewardsForIndex( quest, todaysIndex )
		BattlePassReward todaysBpReward = ItemFlavorBagToBattlePassRewardByIndex( todaysRewardBag, 0 )
		file.todaysRewardData = todaysBpReward
		BattlePass_PopulateRewardButton( todaysBpReward, todaysButton, hasPickedUpTodays, false )

		ToolTipData todaysToolTip
		todaysToolTip.titleText = GetBattlePassRewardHeaderText( todaysBpReward )
		todaysToolTip.descText = GetBattlePassRewardItemName( todaysBpReward )
		todaysToolTip.tooltipFlags = eToolTipFlag.SOLID
		todaysToolTip.rarity = ItemFlavor_GetQuality( todaysBpReward.flav )
		Hud_SetToolTipData( todaysButton, todaysToolTip )
	}
	...
	int EXTRA_ROWS_TO_SHOW = 1
	int gridIndex = int(Clamp((floor(todaysIndex / 7.0) * (EXTRA_ROWS_TO_SHOW * 7)) - 7,0, float(maxBoxCount)))
