...
global function JumpToStoreLegendsTab
global function JumpToStoreCharacter
global function JumpToStoreEditions
...
void function OnGRXStoreUpdate()
{
	...
	else
	{
		...
		foreach ( TabDef tabDef in GetPanelTabs( file.storePanel ) )
		{
			...
			{
				tabDef.title = haveActiveCollectionEvent ? "#MENU_STORE_PANEL_ITEMS" : "#MENU_STORE_FEATURED"
			}
		...
		if( uiGlobal.activeMenu == s_vc.vcPopUpMenu )
...
{
	while ( uiGlobal.activeMenu == s_vc.vcPopUpMenu )
	{
...
void function InitStoreCharactersPanel( var panel )
{
	...
	AddPanelFooterOption( panel, LEFT, BUTTON_A, false, "#A_BUTTON_UNLOCK", "#A_BUTTON_UNLOCK", null, ShouldShowCharacterSelectFooterUnlockButton )
	AddPanelFooterOption( panel, LEFT, BUTTON_X, false, "#X_BUTTON_TOGGLE_LOADOUT", "#X_BUTTON_TOGGLE_LOADOUT", OpenFocusedCharacterSkillsDialog, ShouldShowCharacterSelectFooterSkillsButton )
}
...
void function UpdateCharacterButtons()
{
	...
	foreach ( ItemFlavor character in GetAllCharacters() )
	{
		...
		if ( !isAvailable )
		{
			if ( !ItemFlavor_ShouldBeVisible( character, GetLocalClientPlayer() ) )
				continue
		}
	...
	foreach ( index, character in orderedCharacters )
	{
		...
		else
		{
			...
			if ( offers.len() > 1 )
				Warning( "More than one offer detected for a character in store" )

			                                                                                       
			bool isCharacterPurchaseOfferAvailable = true
			if ( offers.len() > 0 )
			{
				GRXScriptOffer scriptOffer = offers[0]
				if ( !scriptOffer.isAvailable )
					isCharacterPurchaseOfferAvailable = false
			}

			Hud_SetEnabled( button, true )
			RuiSetBool( Hud_GetRui( button ), "isPurchasable", offers.len() > 0 && isCharacterPurchaseOfferAvailable )
			Hud_SetLocked( button, offers.len() == 0 || !isCharacterPurchaseOfferAvailable )
		}
...
bool function ShouldShowCharacterSelectFooterUnlockButton()
{
	if ( !GRX_IsInventoryReady() )
		return false
	...
	{
		ItemFlavor character = s_characters.buttonToCharacter[focus]
		array<GRXScriptOffer> offers = GRX_GetItemDedicatedStoreOffers( character, "character" )
...
bool function ShouldShowCharacterSelectFooterSkillsButton()
{
	if ( !GRX_IsInventoryReady() )
		return false
...
void function CharacterButton_OnGetFocus( var button )
{
	...
	if ( s_characters.buttonWithFocus != button )
		RuiSetGameTime( s_characters.characterSelectInfoRui, "initTime", ClientTime() )
	...
	else
	{
		...
		bool isPurchasableAtAll         = (offers.len() > 0) && isCharacterPurchaseOfferAvailable
...
void function JumpToStoreLegendsTab()
{
	JumpToStoreTab()
...
void function JumpToStoreEditions()
{
	JumpToStoreTab()
	...
	if ( IsDLCStoreInitialized() )
	{
		...
		ActivateTab( storeTabData, Tab_GetTabIndexByBodyName( storeTabData, "SKUShopPanel" ) )
	}
}
