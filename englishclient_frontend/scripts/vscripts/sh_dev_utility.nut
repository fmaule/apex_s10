...
void function DEV_DumpPlayers( float drawDuration = 180 )
{
	array<entity> players = GetPlayerArray()
	printt( "---", FUNC_NAME(), players.len(), "players total ---" )
	if ( players.len() == 0 )
		return

	players.sort( int function( entity lhs, entity rhs )
		{
			int rv = 0
	...
	foreach( idx, player in players )
	{
		...
		if ( playerTeam != lastTeam )
		{
			...
			printt( "" )
			printt( "Team", playerTeam, "has", teamCount, "player(s)" )
			lastTeam = playerTeam
		}
		...
		printf( "  %s[%2d] %-20s", botString, player.GetEntIndex(), player.GetPlayerName() )
...
void function DEV_DrawBoundingBox()
{
	...
	while ( cornerIdx < 4 )
	{
		...
		if ( cornerIdx == 0 )
		{
			...
			DebugDrawMark( fourCorners[0], 20, [255, 255, 255], false, 0.0 )
		...
		else if ( cornerIdx >= 1 )
		{
			...
			DebugDrawLine( fourCorners[0], fourCorners[1], 0, 255, 0, false, 0.0 )
			...
			if ( cornerIdx >= 2 )
			{
				...
				DebugDrawLine( fourCorners[0], fourCorners[2], 255, 0, 0, false, 0.0 )
				...
				if ( cornerIdx == 3 )
				{
					...
					DebugDrawLine( fourCorners[0], fourCorners[3], 0, 0, 255, false, 0.0 )
	...
	DebugDrawRotatedBox( boxCenter, boxMins, boxMaxs, boxAng, 255, 255, 255, true, 20.0 )
