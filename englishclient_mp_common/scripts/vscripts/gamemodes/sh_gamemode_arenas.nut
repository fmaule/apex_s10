...
global function IsRampartTakeoverActive
      
              
global function IsS10E06Active
...
global const int ARENAS_BACKGROUND_SKIN_INDEX_RAMPART_TAKEOVER = 20
global const int ARENAS_SMOKE_SKIN_INDEX_RAMPART_TAKEOVER = 22

global const string RAMPART_ARENAS_TAKEOVER_IDENTIFIER = "Rampart"
...
const vector ARENAS_ROOM_RAMPART_TAKEOVER_SPARKS_COLOR = <255, 0, 110>
...
const asset HOLOGRAM_FX_LEIGH = $"P_holo_glitch_readyroom"
const asset ARENAS_LEIGH = $"P_arenas_menu_ash_human_glitch_CP"
...
void function ShGamemodeArenas_Init()
{
	...
	PrecacheParticleSystem ( HOLOGRAM_FX_LEIGH )
	PrecacheParticleSystem( ARENAS_LEIGH )
	...
	RegisterSignal( "Arenas_EndMenuGlitchThread" )
...
void function EntitiesDidLoad()
{
	...
	if ( IsS10E06Active() )
		SurvivalCommentary_SetHost( eSurvivalHostType.ARENAS_GLITCH )
	else
...
void function Arenas_ShowBuyMenu( int leftTeam, int rightTeam, bool playIntroTransition, float delay = 0.0 )
{
	...
	if ( IsS10E06Active() )
		RuiSetBool ( file.roundStartRui, "isGlitchActive", true )
...
void function _UpdateRoundSummaryAshEffect( bool roundWon )
{
	...
	if ( IsS10E06Active() )
		clGlobal.levelEnt.Signal("Arenas_EndMenuGlitchThread")
...
void function ClSurvivalArenas_OnScoreboardCreated()
{
	...
	if ( IsS10E06Active() )
		RuiSetBool ( file.arenasScoreRui, "isGlitchActive", true )
...
void function UpdateMenuBackground( entity backgroundModel, entity smokeModel, var backgroundRui )
{
	...
	if ( file.roomSparks == -1 )
	{
		...
		file.roomSparks = StartParticleEffectInWorldWithHandle( pIndex, backgroundModel.GetOrigin(), AnglesCompose( backgroundModel.GetAngles(), <0,0,0> ) )
			...
			if ( IsRampartTakeoverActive() )
				EffectSetControlPointVector( file.roomSparks, 1, ARENAS_ROOM_RAMPART_TAKEOVER_SPARKS_COLOR )
			else
...
void function SpawnAsh( vector ashColor, float heightOffset, float sideOffset, float zRotation )
{
	...
	#if NX_PROG
		vector origin = backgroundModel.GetOrigin() + ( 70 * up) + ( forwardOffsetNX * fwd ) + ( -100 * side )
	...
	if ( IsS10E06Active() )
	{
		thread S10E06_MenuGlitchEffectThread ( origin, AnglesCompose( VectorToAngles( newFwd ), <0, 0, 0> ), ashColor )
	}
	else
...
void function S10E06_MenuGlitchEffectThread( vector origin, vector angles, vector ashColor )
{
	int pIndex = GetParticleSystemIndex( ARENAS_ASH )
	int pGlitchIndex = GetParticleSystemIndex( ARENAS_LEIGH )
	clGlobal.levelEnt.Signal("Arenas_EndMenuGlitchThread")
	clGlobal.levelEnt.EndSignal("Arenas_EndMenuGlitchThread")
	
	while ( true )
	{
		file.ashParticle = StartParticleEffectInWorldWithHandle( pIndex, origin, angles )
		EffectSetControlPointVector( file.ashParticle, 1, ashColor )
		wait RandomIntRange( 3, 5 )
		EffectStop ( file.ashParticle, true, true )

		file.ashParticle = StartParticleEffectInWorldWithHandle( pGlitchIndex, origin, angles )
		EffectSetControlPointVector( file.ashParticle, 1, ashColor )
...
int function GetArenasBackgroundSkinIndex()
{
	...
	if ( IsRampartTakeoverActive() )
		return ARENAS_BACKGROUND_SKIN_INDEX_RAMPART_TAKEOVER
...
int function GetArenasSmokeSkinIndex()
{
		...
		if ( IsRampartTakeoverActive() )
			return ARENAS_SMOKE_SKIN_INDEX_RAMPART_TAKEOVER
...
bool function IsRampartTakeoverActive()
{
	return GetCurrentPlaylistVarString( "arenas_takeover_name", "" ) == RAMPART_ARENAS_TAKEOVER_IDENTIFIER
}
      

              
bool function IsS10E06Active()
{
	int unixTimeNow = GetUnixTimestamp()
	if ( unixTimeNow >=  expect int( GetCurrentPlaylistVarTimestamp( "s10e06_active", UNIX_TIME_FALLBACK_2038 ) ) )
	{
		return true
	}
