...
void function Survival_MinimapPackage_ObjectiveAreaInit( entity ent, var rui )
{
	...
	switch ( ent.GetTargetName() )
	{
		...
		case "s10e04_raven_minimap":
			RuiSetColorAlpha( rui, "objColor", SrgbToLinear( <242, 224, 0> / 255.0 ), 0.15 )
			RuiSetColorAlpha( rui, "objBorderColor", SrgbToLinear( <242, 224, 0> / 255.0 ), 0.25 )
			RuiSetBool( rui, "blink", false )
			RuiSetBool( rui, "borderBlink", false )
			RuiSetImage( rui, "centerImage", $"rui/hud/minimap/icon_minimap_white_raven" )
		break
...
void function ToggleFireSelect( entity player )
{
	...
	if ( weapon.GetWeaponClassName() == "mp_weapon_dragon_lmg" && DoesModExist( weapon, "energized_with_thermite" ) )
