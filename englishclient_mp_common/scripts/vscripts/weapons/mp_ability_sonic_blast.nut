...
const string SONIC_BLAST_MARKER_SCRIPT_NAME = "sonic_blast_marker_script"
...
void function MpAbilitySonicBlast_Init()
{
	...
	#if CLIENT
		AddCreateCallback( "prop_script", OnClientSonicBlastMarkerCreated )
	#endif         
...
bool function ShouldBlastHitVictim( vector startPos, vector blastVector, vector blastVecNormalized, entity victim )
{
	                                   
	vector blastStartToVictimNormalized = Normalize( victim.EyePosition() - startPos )

	if ( DotProduct( blastVecNormalized, blastStartToVictimNormalized ) < 0 )
	{
		return false
	}

	float distanceToVictim = Distance( startPos, victim.EyePosition() )

	if ( distanceToVictim > file.sonicBlastRange )
	{
		return false
	}
	...
	float distanceToProjectionSqr = DistanceSqr( projection, victim.GetOrigin() )

	if ( distanceToProjectionSqr <= file.sonicBlastRadiusSqr )
	{
		return true
	}
...
void function OnClientSonicBlastMarkerCreated( entity marker )
{
	if ( marker.GetScriptName() != SONIC_BLAST_MARKER_SCRIPT_NAME )
		return

	entity localViewPlayer = GetLocalViewPlayer()

	vector startPos = marker.GetOrigin()
	vector dir = marker.GetUpVector()
	vector blastVector = startPos + ( dir * file.sonicBlastRange )
	bool isEnemy = IsEnemyTeam( marker.GetTeam(), localViewPlayer.GetTeam() )

	thread DoSonicBlastClientSound_Thread( startPos, blastVector )

	if ( isEnemy )
	{
		if ( localViewPlayer != marker.GetBossPlayer() )
		{
			thread DoSonicBlastThreatIndicator_Thread( localViewPlayer, startPos, blastVector, marker )
		}
	}
}

void function DoSonicBlastClientSound_Thread( vector startPos, vector blastVector )
{
	Assert ( IsNewThread(), "Must be threaded off." )

	var formSound = EmitSoundFromLine( startPos, blastVector, SONIC_BLAST_CYLINDER_FORM_3P )

	float blastDelay =  GetWeaponInfoFileKeyField_GlobalFloat( "mp_ability_sonic_blast", "sonic_blast_delay" )
...
void function DoSonicBlastThreatIndicator_Thread( entity victim, vector startPos, vector blastVector, entity marker )
{
	Assert ( IsNewThread(), "Must be threaded off." )
	victim.EndSignal( "OnDestroy" )
