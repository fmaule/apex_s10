...
void function DeactivateHeartbeatSensor( entity player, bool fromTac )
{
		...
		if ( player == GetLocalViewPlayer() )
		{
			...
			if ( file.heartbeatsHeardWhileActive > 0 )
			{
...
void function UpdateDataForHeartseekerRadarWaveformRadial( entity player, entity victim, bool inTacRange, bool distPercentOverride=false )
{
	...
	if ( inTacRange )
	{
		vector startPos 		  = player.GetWorldSpaceCenter() + ( player.GetViewVector() * SONIC_BLAST_IN_FRONT_START_DISTANCE )
		...
		isLocked = ShouldBlastHitVictim( startPos, blastVector, blastVecNormalized, victim )
	}
	else
	{
