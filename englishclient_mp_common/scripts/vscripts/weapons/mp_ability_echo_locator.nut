...
struct
{
	...
	table<entity, dummieVelocityData> dummieVelocity
	#endif         
	#if SERVER
...
float function GetPlayerSpeedForEchoLocator( entity player )
{
	...
	{
		if ( player in file.dummieVelocity )
		{
			playerSpeed = file.dummieVelocity[player].speed
		}
	}
	else
	{
...
void function EchoLocatorFootstepVFX_Thread( entity echoLocator )
{
	...
	while ( true )
	{
		...
		if ( IsFiringRangeGameMode() )
		{
			...
			foreach ( entity dummy in dummies )
			{
				if ( IsValid( dummy ) )
				{
					...
					if ( !( dummy in file.dummieVelocity ) )
					{
						dummieVelocityData data
						data.time                  = Time()
						data.origin                = dummy.GetOrigin()
						data.speed                 = 0
						file.dummieVelocity[dummy] <- data
					}
				}
			}
		}
		...
		if ( IsFiringRangeGameMode() )
		{
			...
			foreach ( entity dummy, dummieVelocityData data in file.dummieVelocity )
			{
				if ( IsValid( dummy ) )
				{
					if ( IsAlive( dummy ) )
					{
						vector distDiff = dummy.GetOrigin() - data.origin
						float timeDelta = Time() - data.time
						float speed     = Length( distDiff ) / timeDelta
...
{
	int particleSystemID = GetParticleSystemIndex( ECHO_LOCATOR_VISUAL_FOOT_PING )
	...
	EffectSetControlPointVector( handle, 1, ENEMY_COLOR_FX )
}
