...
const string VEND_PANEL = "rampart_tt_vend_panel"
...
{
	if( Crafting_GetPlayerCraftingMaterials( player ) >= Vend_GetCost( panel ) )
	{
		return true
	}
...
entity function GetRampartTTPanelForLoot( entity lootEnt )
{
	foreach ( entity panel in GetEntArrayByScriptName( VEND_PANEL ) )
	{
		if(IsValid ( panel ) )
		{
			if( DistanceSqr( panel.GetOrigin(), lootEnt.GetOrigin() ) < DISTANCE_PANEL_TO_LOOT_SQR )
				return panel
		}
	}
